...
/**
  ...
  * @param      p_user_ param of user,input to grab image thread
  */ 
void* GrabThread(void *p_user_) {
  int n_ret = MV_OK;
  // ch:获取数据包大小 | en:Get payload size
  MVCC_INTVALUE stParam;
  memset(&stParam, 0, sizeof(MVCC_INTVALUE));
  n_ret = MV_CC_GetIntValue(((PUser*)p_user_)->handle_, "PayloadSize", &stParam);
  if (MV_OK != n_ret) {
  ...
  while(true) {
    ...
    n_ret = MV_CC_GetOneFrameTimeout(((PUser*)p_user_)->handle_, pData, nDataSize, &stImageInfo, 1000);
    if (n_ret == MV_OK) {
      ((PUser*)p_user_)->image_ = cv::Mat(stImageInfo.nHeight,stImageInfo.nWidth,CV_8UC3,pData);
    } else {
      printf("No data[%x]\n", n_ret);
      continue;
    }
...
int32_t Camera::Open() {
  ...
  pthread_t nThreadID;
  p_user_.handle_ = handle_;
  n_ret = pthread_create(&nThreadID, NULL, GrabThread, (void*)(&p_user_));
...
cv::Mat Camera::Grab(bool is_online = true) {
  ...
  cv::cvtColor(p_user_.image_,image_,cv::COLOR_RGB2BGR);
  is_grab_finished = false;
  return image_;
}
