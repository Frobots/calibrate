...
int32_t ExternalParamCalibrate::StartCalibrate() {
  ...
  // input four points
  #if 1
    std::ifstream infile3("./config/InputPoints.txt");
    if (!infile3.is_open()) {
      std::cout << "open InputPoints.txt failed!" << std::endl; 
      return -1;
    } else {
      int32_t rows = 0;
      while (!infile3.eof()) {
        char line[1024] = { 0 };
	      infile3.getline(line, 1024);
	      std::stringstream word(line);
        if (rows < 4) {
          cv::Point2f point;
          word >> point.x >> point.y;
          // std::cout << point << std::endl;
          image_points_.push_back(point);
        } else {
          cv::Point3f point;
          word >> point.x >> point.y >> point.z;
